...
# keeps=['concept:name','Player','case:concept:name','time:timestamp','attribute:x'
...
# str_columns = list(log_df.select_dtypes(include=["object", "string"]).columns)
# num_columns = list(log_df.select_dtypes(include=["number"]).columns)
# columns_to_skip = ["case:concept:name"]#, "time:timestamp","concept:name"]
# str_columns = list(set(str_columns) - set(columns_to_skip))
# num_columns = list(set(num_columns) - set(columns_to_skip))

# from sklearn.model_selection import train_test_split
# #X_train, X_test, y_train, y_test = train_test_split(X, y, stratify=y, random_state=42)
# def log_train_test_split(log_df, base='case:concept:name'):
#     uniques=np.unique(log_df[base])
#     train, test = train_test_split(uniques)
#     train_df = log_df[log_df[base].isin(train)]
#     test_df = log_df[log_df[base].isin(test)]
#     return train_df, test_df

# from sklearn.decomposition import PCA
# log_features, feature_names_log = log_to_features.apply(log, variant=trace_based)
# #log_features, feature_names_log = log_to_features.apply(log, parameters={'STR_EVENT_ATTRIBUTES':str_columns, 'NUM_EVENT_ATTRIBUTES': num_columns})
# log_features_df = pd.DataFrame(log_features, columns=feature_names_log)
# log_features_df = log_features_df.fillna(0)
# pca = PCA(n_components=200)
# pca_df = pd.DataFrame(pca.fit_transform(log_features_df))
